...
const Aria2 = {
  ...
  async start() {
    ...
    const userAgent = Config.read('userAgent')
  ...
  async isGIDExisting(gid) {
    try {
      // '[]' means we don't really want to know anything from Aria2.
      await aria2client.call('tellStatus', gid, [])
      return true
    } catch (error) {
      // Definition of GID:
      // https://aria2.github.io/manual/en/html/aria2c.html#cmdoption-gid
      if (error.code === 1 && /^GID [0-9a-fA-F]{16} is not found$/.test(error.message)) {
        return false
      }

      // Otherwise this is an exception we can't handle.
      throw error
    }
  },
}
